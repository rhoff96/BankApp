...
public class JdbcTransactionDaoTests extends BaseDaoTests {
    ...
    private static final Transaction TRANSACTION_4 = new Transaction(Timestamp.valueOf("2023-02-02 00:00:00"), new BigDecimal("100.0"), 13, 2, new BigDecimal("200.0"));

    private static final Transaction TRANSACTION_5 = new Transaction(Timestamp.valueOf("2015-11-20 00:00:00"), new BigDecimal("50.0"), 2, 2, new BigDecimal("700.0"));
    ...
    public void when_valid_transaction_then_create_transaction_creates_transaction() {
        ...
        newTransaction.setTime(Timestamp.valueOf("2023-02-02 00:00:00"));
        newTransaction.setAmount(new BigDecimal("200.0"));
        newTransaction.setPreviousBalance(new BigDecimal("100.0"));
    ...
    public void when_account_number_given_and_amount_not_0_then_generate_report_returns_transactions_for_account_and_amount() {
        List<Transaction> expected = new ArrayList<>();
        expected.add(TRANSACTION_5);
        List<Transaction> actual = testTd.generateReport(Timestamp.valueOf("2000-01-01 00:00:00"), Timestamp.valueOf("2023-12-01 00:00:00"), 2, new BigDecimal("700.0"));
        Assert.assertEquals("Account number and balance of 0 should return all transactions", expected, actual);
    }
    @Test
    ...
    public void when_account_number_0_and_amount_not_0_then_generate_report_returns_transactions_for_all_accounts_with_given_amount() {
        List<Transaction> expected = new ArrayList<>();
        expected.add(TRANSACTION_5);
        List<Transaction> actual = testTd.generateReport(Timestamp.valueOf("2000-01-01 00:00:00"), Timestamp.valueOf("2023-12-01 00:00:00"), 0, new BigDecimal("700.0"));
        Assert.assertEquals("Account number and balance of 0 should return all transactions", expected, actual);
    }
}
